...
CREATE TABLE IF NOT EXISTS student_award_applications (
    ...
    material_path VARCHAR(255) COMMENT '申请材料文件路径',
    material_name VARCHAR(255) COMMENT '申请材料文件名',
    material_size BIGINT COMMENT '申请材料文件大小（字节）',
    material_type VARCHAR(100) COMMENT '申请材料文件类型',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    FOREIGN KEY (student_id) REFERENCES students(id) ON DELETE CASCADE,
    FOREIGN KEY (award_id) REFERENCES awards(id) ON DELETE CASCADE
