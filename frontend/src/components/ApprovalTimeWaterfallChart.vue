...
// 绘制图表
    const drawChart = async () => {
      try {
        ...
        // 处理空数据情况
        const categories = data ? Object.keys(data) : ['申请到教师审批', '教师审批到管理员审批', '管理员审批到最终结果']
        const values = data ? Object.values(data) : [0, 0, 0]
        ...
        try {
          ...
          if (chartInstance) {
            const errorOption = {
              ...
              xAxis: {
                type: 'category',
                data: ['申请到教师审批', '教师审批到管理员审批', '管理员审批到最终结果']
              },
              yAxis: {
                type: 'value'
              },
